<?php
require 'vendor/autoload.php'; // Pastikan sudah install phpoffice/phpspreadsheet
require 'db_connect.php';
session_start();

use PhpOffice\PhpSpreadsheet\IOFactory;
use PhpOffice\PhpSpreadsheet\Style\Alignment;
use PhpOffice\PhpSpreadsheet\Style\Border;

if (!isset($_GET['id'])) {
    die("ID Sesi tidak ditemukan.");
}

$id_session = (int)$_GET['id'];

// 1. Ambil Metadata Sesi & Training
$meta_sql = "
    SELECT t.nama_training, ts.date_start, ts.date_end, ts.instructor_name, ts.location, ts.time_info, ts.provider
    FROM training_session ts 
    JOIN training t ON ts.id_training = t.id_training 
    WHERE ts.id_session = $id_session
";
$meta = $conn->query($meta_sql)->fetch_assoc();

// 2. Ambil Rata-rata Feedback (Satisfaction)
$feedback_sql = "
    SELECT AVG(statis_subject) as avg_sub, AVG(instructor) as avg_inst, AVG(statis_infras) as avg_inf, COUNT(id_score) as total_p
    FROM score WHERE id_session = $id_session
";
$fb = $conn->query($feedback_sql)->fetch_assoc();

// 3. Ambil Data Peserta
$list_sql = "
    SELECT k.index_karyawan, k.nama_karyawan, f.func_n1 as jabatan, s.pre, s.post
    FROM score s
    JOIN karyawan k ON s.id_karyawan = k.id_karyawan
    LEFT JOIN func f ON s.id_func = f.id_func
    WHERE s.id_session = $id_session
    ORDER BY k.nama_karyawan ASC
";
$participants = $conn->query($list_sql);

// --- PROSES EXCEL ---

$templateFile = 'Training Reports.xlsx'; // Pastikan file template ada di folder yang sama
$spreadsheet = IOFactory::load($templateFile);
$sheet = $spreadsheet->getActiveSheet();

// 4. Isi Header (Sesuai Snippet Template Anda)
$sheet->setCellValue('C7', ': ' . $meta['nama_training']);
$sheet->setCellValue('C8', ': ' . date('d M Y', strtotime($meta['date_start'])));
$sheet->setCellValue('C9', ': ' . $fb['total_p'] . ' Orang');
$sheet->setCellValue('C10', ': ' . ($meta['instructor_name'] ?? '-'));

$sheet->setCellValue('F7', ': ' . ($meta['provider'] ?? '-'));
$sheet->setCellValue('F8', ': ' . ($meta['location'] ?? '-'));
$sheet->setCellValue('F9', ': ' . ($meta['time_info'] ?? '-'));

// 5. Isi Feedback (Jika ada di template baris 12-14)
$sheet->setCellValue('C12', number_format($fb['avg_sub'], 1));
$sheet->setCellValue('C13', number_format($fb['avg_inst'], 1));
$sheet->setCellValue('C14', number_format($fb['avg_inf'], 1));

// 6. Isi Tabel Peserta (Mulai Baris 17)
$row = 17;
$no = 1;
while ($p = $participants->fetch_assoc()) {
    $sheet->setCellValue('A' . $row, $no);
    $sheet->setCellValue('B' . $row, $p['index_karyawan']);
    $sheet->setCellValue('C' . $row, $p['nama_karyawan']);
    $sheet->setCellValue('D' . $row, $p['jabatan']);
    $sheet->setCellValue('E' . $row, $p['pre']);
    $sheet->setCellValue('F' . $row, $p['post']);
    
    // Keterangan Lulus (Contoh standar 75)
    $keterangan = ($p['post'] >= 75) ? "Lulus" : "Remedial";
    $sheet->setCellValue('G' . $row, $keterangan);

    // Tambahkan border manual agar rapi saat baris bertambah
    $sheet->getStyle("A$row:G$row")->getBorders()->getAllBorders()->setBorderStyle(Border::BORDER_THIN);
    
    $row++;
    $no++;
}

// 7. Output ke Browser
$filename = "Report_" . str_replace(' ', '_', $meta['nama_training']) . ".xlsx";

header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header('Content-Disposition: attachment;filename="'. $filename .'"');
header('Cache-Control: max-age=0');

$writer = IOFactory::createWriter($spreadsheet, 'Xlsx');
$writer->save('php://output');
exit();